...
def train(model, train_dataloader, val_dataloader, epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        for field, candidate, words, positions, masks, labels in tqdm(
            train_dataloader, desc="Epoch %s" % epoch
        ):
            ...
            val_accuracy, val_loss, val_recall, val_precision = evaluate(
            ...
            print(
                f"Metrics for Epoch {epoch}:  Loss:{round(train_loss, 4)} \
...
if __name__ == "__main__":
    ...
    validation_data = data.DataLoader(
        val_data, batch_size=config.BATCH_SIZE, shuffle=True
    )
    ...
    if config.PLOT:
        ...
        history_df.plot(y=["training_loss", "validation_loss"], use_index=True)
        plt.suptitle("ReLie")
        plt.show()
