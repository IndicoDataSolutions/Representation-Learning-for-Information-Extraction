    ...
    for cls, both_cads in candidates.items():
        for cad in both_cads:
            neighbours = Neighbour.find_neighbour(
                cad, words, x_offset, y_offset, width, height
            )
        ...
        if annotations[field]:
            for idx, val in enumerate(value):
                _neighbours, _neighbour_cords = preprocess.get_neighbours(
                    val["neighbours"], vocabulary, n_neighbours
                )
                field_ids.append(np.eye(n_classes)[fields_dict[field]])
                candidate_cords.append([val["x"], val["y"]])
...
def parse_args():
    ...
    parser.add_argument(
        "--image_path", dest="image_path", help="directory to load models", type=str
    )
    parser.add_argument(
        "--visualize",
        dest="visualize",
...
def main():
    ...
    candidates_with_neighbours = attach_neighbour_candidates(
        width, height, ocr_results, candidates
    )
    ...
    with torch.no_grad():
        rlie.eval()
        val_outputs = rlie(
            field_ids, candidate_cords, neighbours, neighbour_cords, masks=None
        )
    val_outputs = val_outputs.to("cpu").numpy()
    out = {
        cl: np.argmax(val_outputs[np.where(field_idx_candidate == cl)])
        for cl in np.unique(field_idx_candidate)
    }
