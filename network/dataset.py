...
class DocumentsDataset(data.Dataset):
    ...
    def __init__(self, split_name="train"):
        ...
        if cached_data_path.exists():
            ...
            (
                self.field_ids,
                self.candidate_cords,
                self.neighbours,
                self.neighbour_cords,
                self.mask,
                self.labels,
            ) = _data
        else:
            print("Preprocessed data not available")
            annotation, classes_count, class_mapping = xml_parser.get_data(
                config.XML_DIR, split_name
            )
            print("Class Mapping:", class_mapping)
            print("Class counts:", classes_count)
            annotation = candidate.attach_candidate(annotation, config.CANDIDATE_DIR)
            annotation, self.vocab = Neighbour.attach_neighbour(
                annotation, config.OCR_DIR, vocab_size=config.VOCAB_SIZE
            )
            annotation = op.normalize_positions(annotation)
            _data = preprocess.parse_input(
                annotation, class_mapping, config.NEIGHBOURS, self.vocab
            )
            (
                self.field_ids,
                self.candidate_cords,
